...
class BafangData extends ChangeNotifier {
  ...
  void injectSimFrames() {
    update0601(Uint8List.fromList([
      0x00, 0x00, 0x00, 0x01, 0x00, 0x01, 0x09, 0x51,
      0x00, 0x1f, 0x10, 0x3c, 0x2d, 0x00, 0x00, 0x3a,
      0x6e, 0x00, 0x00, 0x00, 0x00,
    ]));
    update0609(Uint8List.fromList([
      0xfd, 0x51, 0x00, 0x00, 0xc1, 0x07, 0x5a, 0x11,
      0x18, 0x01, 0xde, 0x03, 0x55, 0x00, 0x01, 0x05,
    ]));
  ...
  bool injectHex(String hex) {
    try {
      final bytes = Uint8List.fromList(
        hex.trim().split(RegExp(r'\s+')).map((s) => int.parse(s, radix: 16)).toList(),
      );
      ...
      if (bytes.length >= 9 && bytes[0] == 0x55 && bytes[1] == 0xaa) {
        ...
        if (bytes.length == 9 + payloadLen) {
          ...
          if (op == 0x06 && reg == 0x01) { update0601(data); return true; }
          if (op == 0x06 && reg == 0x09) { update0609(data); return true; }
        }
      }
      // Fallback: treat as raw DATA bytes for 06 01 if length == 21
      if (bytes.length == 21) { update0601(bytes); return true; }
      if (bytes.length == 16) { update0609(bytes); return true; }
    } catch (_) {}
    return false;
  }
